...
| Scenes measured at once | 4 | how many scenes a task measures in parallel (1 to 16): as many threads overlap the frame decodes and as many worker processes run the arithmetic; on an 8K library on a network share, 4 ran the detail task about 2.8 times and the retag measurement about 2.7 times faster than 1, and 8 a little faster again (reading the files from the share is then the limit); each worker reads its own file from the media storage, so set 1 for a slow network share or a busy server. The scan hook always measures one scene. Resuming, the log and the progress stay in scene order. |
